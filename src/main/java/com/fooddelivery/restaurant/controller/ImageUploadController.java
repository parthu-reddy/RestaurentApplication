...
public class ImageUploadController {
    ...
    public ResponseEntity<ApiResponse<String>> uploadImage(@RequestParam("file") MultipartFile file, @RequestParam("folderId") String folderId, @RequestParam(value = "imageType", defaultValue = "default") String imageType, Authentication authentication) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.error("Failed to upload image: " + e.getMessage() + " (UPLOAD_FAILED)"));
        }
    }
    ...
    private byte[] compressImage(byte[] originalBytes, String imageType) throws Exception {
        ...
        try (java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream()) {
            // Honors max size, won't upscale if smaller
            net.coobird.thumbnailator.Thumbnails.of(originalImage).size(maxWidth, maxWidth).outputFormat("jpg").outputQuality(0.9F).toOutputStream(baos);
        ...
        for (int i = 0; i < 5; i++) {
            try (java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream()) {
                net.coobird.thumbnailator.Thumbnails.of(originalImage).size(maxWidth, maxWidth).outputFormat("jpg").outputQuality(quality).toOutputStream(baos);
