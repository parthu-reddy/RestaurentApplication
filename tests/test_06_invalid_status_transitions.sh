...
echo "--- START: Invalid Status Transitions Scenario ---"

echo "1. Onboarding Restaurant..."
REST_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"name": "Invalid Burger", "fssaiLicenseNumber": "1234567896", "gstin": "123456789012352", "pan": "ABCDE1240F", "cin": "U12345MH2023PTC123462", "bankAccountNumber": "1234567890", "ifscCode": "HDFC0001234", "lat": 12.9716, "lng": 77.5946}' http://localhost:8092/api/v1/restaurants/onboard)
REST_ID=$(echo $REST_RESPONSE | jq -r '.data.id')

echo "2. Adding Menu Item..."
MENU_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"name": "Invalid Meal", "price": 150.00, "isAvailable": true}' http://localhost:8092/api/v1/restaurants/$REST_ID/catalog/items)
...
DEL_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d "{\"name\": \"Invalid Exec\", \"phoneNumber\": \"$DEL_PHONE\", \"vehicleNumber\": \"KA01AB9999\"}" http://localhost:8092/api/delivery/onboard)
...
echo "6. Invalid Attempt: Restaurant tries to mark READY_FOR_PICKUP before PAID/ACCEPTED..."
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:8092/api/v1/restaurants/$REST_ID/fulfillment/orders/$ORDER_ID/ready)
if [ "$HTTP_STATUS" != "500" ] && [ "$HTTP_STATUS" != "400" ]; then
    echo "FAIL: Expected HTTP error for invalid transition, got $HTTP_STATUS"
    exit 1
else
    echo "Successfully caught invalid transition attempt for READY_FOR_PICKUP (HTTP $HTTP_STATUS)"
fi

echo "7. Simulating Payment Webhook..."
...
echo "8. Polling for PAID Status..."
MAX_RETRIES=20
...
if [ "$HTTP_STATUS" != "500" ] && [ "$HTTP_STATUS" != "400" ]; then
    ...
    echo "Successfully caught invalid transition attempt for DELIVERED (HTTP $HTTP_STATUS)"
fi

echo "--- SUCCESS: Invalid Status Transitions Scenario Completed ---"
exit 0
