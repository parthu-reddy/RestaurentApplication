...
echo "--- START: Restaurant Rejection Scenario ---"

echo "1. Onboarding Restaurant..."
REST_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"name": "Reject Burger", "fssaiLicenseNumber": "1234567891", "gstin": "123456789012346", "pan": "ABCDE1235F", "cin": "U12345MH2023PTC123457", "bankAccountNumber": "1234567890", "ifscCode": "HDFC0001234", "lat": 12.9716, "lng": 77.5946}' http://localhost:8092/api/v1/restaurants/onboard)
REST_ID=$(echo $REST_RESPONSE | jq -r '.data.id')

echo "2. Adding Menu Item..."
MENU_RESPONSE=$(curl -s -X POST -H "Content-Type: application/json" -d '{"name": "Reject Meal", "price": 150.00, "isAvailable": true}' http://localhost:8092/api/v1/restaurants/$REST_ID/catalog/items)
...
echo "7. Restaurant Rejects Order..."
curl -s -X POST http://localhost:8092/api/v1/restaurants/$REST_ID/fulfillment/orders/$ORDER_ID/reject > /dev/null

echo "8. Verifying CANCELLED Status..."
for (( i=1; i<=MAX_RETRIES; i++ )); do
  ORDER_STATUS=$(docker exec -i food_delivery_db psql -U postgres -d food_delivery -t -c "SELECT status FROM orders WHERE id = '$ORDER_ID';" | xargs)
  if [ "$ORDER_STATUS" == "CANCELLED" ]; then break; fi
  sleep 2
done

if [ "$ORDER_STATUS" != "CANCELLED" ]; then echo "FAIL: Expected CANCELLED, got $ORDER_STATUS"; exit 1; fi

echo "--- SUCCESS: Restaurant Rejection Scenario Completed ---"
exit 0
